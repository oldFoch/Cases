...
const { REQUIRED_PARAMS, REQUIRED_SIGNED_PARAMS, VALID_PARAMS } = require('./constants');
...
/**
 * Validates that the OpenID endpoint matches Steam's expected endpoint
 * @param {object} query - The query object containing OpenID parameters
 * @returns {string} The validated OpenID endpoint
 * @throws {Error} If the endpoint doesn't match Steam's endpoint
 */
function validateOpEndpoint(query) {
	const validOpEndpoint = 'https://steamcommunity.com/openid/login';
	const opEndpoint = query['openid.op_endpoint'];
	assert(opEndpoint === validOpEndpoint, `OpenID endpoint "${opEndpoint}" does not match expected endpoint "${validOpEndpoint}"`);
...
function sanitizeQuery(query, expectedRealm) {
	// Validate the OpenID endpoint
	validateOpEndpoint(query);
	...
	// validate the sanitized query doesn't contain any other parameters
	const hasInvalidParams = Object.keys(sanitizedQuery).some(param => !VALID_PARAMS.includes(param));
	assert(!hasInvalidParams, 'Sanitized query contains invalid parameters');
